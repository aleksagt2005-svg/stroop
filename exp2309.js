...
let expName = 'exp2309';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'exp23.xlsx', 'path': 'exp23.xlsx'},
  ]
});
...
async function experimentInit() {
  ...
  fetch('https://pipe.jspsych.org/api/data', {
      ...
      headers: {
       ...
       body: JSON.stringify({
          experimentID: 'VR8QQ2QMWzUn', // * DATAPIPE EXP ID*
          filename: filename, 
          data: data,
       }),
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *mask* updates
    if (t >= 0.02 && mask.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 0.02 + 0.5 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    ...
    if (response_keyboard.status === PsychoJS.Status.STARTED) {
      let theseKeys = response_keyboard.getKeys({keyList: ['r','g','b','y'], waitRelease: false});
